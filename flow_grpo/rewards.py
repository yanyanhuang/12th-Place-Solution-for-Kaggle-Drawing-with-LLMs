...
def svg_score(device):
    from flow_grpo.svg_eval import SVGScorer

    scorer = SVGScorer(device=device)
    ...
    def _fn(images, prompts, metadata):
        if isinstance(images, torch.Tensor):
            ...
            images = images.transpose(0, 2, 3, 1)  # NCHW -> NHWC
            images = [Image.fromarray(image) for image in images]
        multiple_choice_qas = [data["multiple_choice_qa"] for data in metadata]
        return scorer(images, multiple_choice_qas), {}
