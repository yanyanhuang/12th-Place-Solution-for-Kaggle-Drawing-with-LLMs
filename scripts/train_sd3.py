...
from flow_grpo.svg_eval import bitmap_to_svg_vtracer
from flow_grpo.svg_image_fidelity import svg_to_png
...
class SVGPromptDataset(Dataset):
    def __init__(self, dataset, split='train'):
        ...
        with open(self.file_path, 'r', encoding='utf-8') as f:
            self.metadatas = [json.loads(line) for line in f]
            self.prompts = [item['description'] for item in self.metadatas]

        self.prefix = "SVG style, simple vector art, flat design, clean lines, sharp edges, minimalist illustration, graphic design, vector graphics,"
        
    def __len__(self):
        return len(self.prompts)
    
    def __getitem__(self, idx):
        return {"prompt": self.prefix + self.prompts[idx], "metadata": self.metadatas[idx]}
...
def eval(pipeline, test_dataloader, text_encoders, tokenizers, config, accelerator, global_step, reward_fn, executor, autocast, num_train_timesteps, ema, transformer_trainable_parameters):
    ...
    neg_prompt_embed, neg_pooled_prompt_embed = compute_text_embeddings(["photorealistic, photography, 3D render, complex shading, intricate details, textures, blur, noise, grainy, painterly, brush strokes, shadows, highlights, fur texture, whiskers."], text_encoders, tokenizers, max_sequence_length=128, device=accelerator.device)
    ...
    if accelerator.is_main_process:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            for idx, index in enumerate(sample_indices):
                ...
                pil_svg = bitmap_to_svg_vtracer(pil)
                pil_svg = svg_to_png(pil_svg)
                pil = pil.resize((config.resolution, config.resolution))
                pil.save(os.path.join(tmpdir, f"{idx}.jpg"))  # 使用新的索引
                pil_svg.save(os.path.join(tmpdir, f"{idx}_svg.jpg"))
            ...
            accelerator.log(
                {
                    ...
                    "eval_svgs": [
                        wandb.Image(
                            os.path.join(tmpdir, f"{idx}_svg.jpg"),
                            caption=f"{prompt:.1000} | " + " | ".join(f"{k}: {v:.2f}" for k, v in reward.items() if v != -10),
                        )
                        for idx, (prompt, reward) in enumerate(zip(sampled_prompts, sampled_rewards))
                    ],
...
def main(_):
    ...
    if config.prompt_fn == "general_ocr":
    ...
    elif config.prompt_fn == "svg":
        train_dataset = SVGPromptDataset(config.dataset, 'train')
        test_dataset = SVGPromptDataset(config.dataset, 'test')
        # 创建无限循环的DataLoader
        train_sampler = DistributedKRepeatSampler( 
            dataset=train_dataset,
            batch_size=config.sample.train_batch_size,
        ...
        train_dataloader = DataLoader(
            ...
            batch_sampler=train_sampler,
            num_workers=1,
            collate_fn=SVGPromptDataset.collate_fn,
            # persistent_workers=True
        )
        # 创建正常的DataLoader
        test_dataloader = DataLoader(
    ...
    for epoch in range(first_epoch, config.num_epochs):
        ...
        for i in tqdm(
            ...
            if i==0 and (epoch-2) % config.eval_freq == 0 and epoch>0:
            ...
            if i==0 and (epoch-2) % config.save_freq == 0 and epoch>0 and accelerator.is_main_process:
